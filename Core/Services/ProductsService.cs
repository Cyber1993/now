...
namespace Core.Services
{
...

    {
        ...
        public async Task Edit(int productId, EditProductDto product)
        {
        ...
        public async Task<PaginatedList<GetProductDto>> GetByFilter(ItemsFilter filter)
        {
            ...
            if (!string.IsNullOrEmpty(filter.SearchTerm))
            {
                ...
                query = query.Where(u => u.Title.Contains(searchTermLower));
            }
            ...
            { 
                {
                    "Name",
                    q => filter.SortDirection == "asc"
                        ? q.OrderBy(product => product.Title)
                        : q.OrderByDescending(product => product.Title)
                },
                {
                    "Description",
                    q => filter.SortDirection == "asc"
                        ? q.OrderBy(product => product.Description)
                        : q.OrderByDescending(product => product.Description)
                },
            };
            ...
            var productsMapped = new List<GetProductDto>();

            foreach (var Product in products)
            {
                var ProductDto = mapper.Map<GetProductDto>(Product);
                productsMapped.Add(ProductDto);
            }
