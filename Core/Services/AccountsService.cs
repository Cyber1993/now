...
namespace Core.Services
{
                                 ...
                                 IImagesService imagesService) : IAccountsService
    {
        ...
        public async Task Edit(string userId, EditUserDto editUserDto)
        {
            var existingUser = await userManager.FindByIdAsync(userId);

            string image = existingUser.Image;

            mapper.Map(editUserDto, existingUser);

            if (editUserDto.Image != null)
            {
                imagesService.DeleteImage(image);
                existingUser.Image = imagesService.SaveImage(editUserDto.Image);
            }
            else
            {
            ...
            if (editUserDto.Roles != null && editUserDto.Roles.Any())
            {
                ...
                var rolesToRemove = existingRoles.Except(editUserDto.Roles);
                await userManager.RemoveFromRolesAsync(existingUser, rolesToRemove);

                var rolesToAdd = editUserDto.Roles.Except(existingRoles);
